    ...
    <div id="navigation_index" class="shadow">
	   <ul id="navmenu-h">
        ...
        <li><?php if($session->check('uid')) echo $html->link(__('Friends',true), array('controller'=>'friends','action'=>'index'));?>
          <ul>
            <li><?php if($session->check('uid')) echo $html->link(__('Add Friends',true), array('controller'=>'friends','action'=>'add'));?></li>
          </ul>
        </li>
		...
		<li>
		<?php if(!$session->check('uid')) echo $html->link('Login','/users/login/');
